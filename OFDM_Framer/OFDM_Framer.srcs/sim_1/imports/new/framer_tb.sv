...
module framer_tb
    ...
    logic  s_axis_ctrl_tlast;
    logic  s_axis_ctrl_tvalid = 0;
 ...
 //           while (input_counter < INPUT_SAMPLES) begin
                ...
                s_axis_ctrl_tdata <= mod_index + 1;
                do @(posedge axis_aclk); while(~s_axis_ctrl_tready);
                s_axis_ctrl_tvalid <= 0;
                s_axis_data_tvalid <= 1;
                for (k = 0; k < 180 * bits_per_mod[mod_index]; k++) begin
                    s_axis_data_tdata <= rand_int;
                    input_counter <= input_counter + 1;
                    do @(posedge axis_aclk); while(~s_axis_data_tready);

                end
                if((repetitions == 1)) begin
                     @(posedge axis_aclk);
                   s_axis_data_tvalid <= 0;
                   repeat (10000) @(posedge axis_aclk);
                end
 //           end
        end
