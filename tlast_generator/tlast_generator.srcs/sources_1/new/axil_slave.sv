module axil_slave_tlast
    ...
    (
		...
		output logic [31 : 0] frame_len,
		output logic reset_len
    );
    ...
    always @(posedge clk) begin
        if(!aresetn) begin
            frame_len_reg <= 32'h0000;
        end
        else begin
            if(write_rdy) begin
                case(write_addr)
                    1'b0 : frame_len_reg <= s_axi_wdata;
                endcase
            end
        end
    end
   ...
   always @(posedge clk) begin
        if(read_rdy) begin
            case(read_addr)
                1'b0 : s_axi_rdata <= frame_len_reg;
            endcase
        end
    end
