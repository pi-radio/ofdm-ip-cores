...
`timescale 1 ns / 1 ps
	...
	logic [1 : 0] max_in_samp_idx_corr1_tmp[0 : 1];
	logic [1 : 0] max_in_samp_idx_corr2_tmp[0 : 1];
	logic signed[2 : 0] max_in_samp_idx_corr1 = -1;
	logic signed[2 : 0] max_in_samp_idx_corr2 = -1;
    ...
    logic in_sample_offset_corr1;
    logic in_sample_offset_corr2;
	...
	generate
	   for(k = 0; k < 4; k = k + 2) begin
	       comparator c(mag_sq_corr1_tmp[k], mag_sq_corr1_tmp[k + 1], k, k + 1, 
	           max_four_corr1[k/2], max_in_samp_idx_corr1_tmp[k/2]);
	   end
	endgenerate
	
	generate
	   for(k = 0; k < 4; k = k + 2) begin
	       comparator c(mag_sq_corr2_tmp[k], mag_sq_corr2_tmp[k + 1], k, k + 1, 
	           max_four_corr2[k/2], max_in_samp_idx_corr2_tmp[k/2]);
	   end
	endgenerate
	...
	always_comb begin
	   in_sample_offset_corr1 <= (max_in_samp_idx_corr1 == 0) ? 0 : 1;
	   in_sample_offset_corr2 <= (max_in_samp_idx_corr2 == 0) ? 0 : 1;
	end
...
//	assign mag_sq_corr2 = (max_four_corr2[0] > max_four_corr2[1]) ? max_four_corr2[0] : max_four_corr2[1];
    ...
    assign log[26 : 0] = mag_sq_corr1;
    assign log[53 : 27] = mag_sq_corr2;
    assign log[54] = state_tx;
    assign log[57 : 55] = state_peak_det_c1;
    assign log[60 : 58] = state_peak_det_c2;
            ...
            case(state_peak_det_c1) 
                 ...
                 SECOND_SEARCH: begin
                    if(max_idx2_c1 > 0 && max_idx2_c1 < 192 
                            ...
                            start_idx_c1 <= (sample_cntr[8]) ? 512 - max_idx2_c1 - in_sample_offset_corr1
                                                 : 256 - max_idx2_c1 - in_sample_offset_corr1;
                            state_peak_det_c1 <= FIRST_SEARCH;
                     end
            ...
            case(state_peak_det_c2) 
                 ...
                 SECOND_SEARCH: begin
                    if((max_idx2_c2 > 0) && (max_idx2_c2 < 192) 
                            ...
                            start_idx_c2 <= (sample_cntr[8]) ? 512 - (max_idx2_c2 + 128) - in_sample_offset_corr1
                                         : 256 - (max_idx2_c2 + 128) - in_sample_offset_corr1;
                        state_peak_det_c2 <= FIRST_SEARCH;
                        end
        ...
        if(s_axis_corr1_tvalid && s_axis_corr2_tvalid && s_axis_data_in_tvalid) begin
            ...
            if(mag_sq_corr1 > (noise_floor + threshold)) begin
                if(state_peak_det_c1 == FIRST_SEARCH && mag_sq_corr1 > max_mag1_c1) begin 
                    ...
                    max_in_samp_idx_corr1 <=  (max_four_corr1[0] > max_four_corr1[1]) ?
                             max_in_samp_idx_corr1_tmp[0] : max_in_samp_idx_corr1_tmp[1];
                end
            ...
            if(mag_sq_corr2 > (noise_floor + threshold)) begin
                if(state_peak_det_c2 == FIRST_SEARCH && mag_sq_corr2 > max_mag1_c2) begin
                    ...
                    max_in_samp_idx_corr2 <=  (max_four_corr2[0] > max_four_corr2[1]) ?
                             max_in_samp_idx_corr2_tmp[0] : max_in_samp_idx_corr2_tmp[1];  
                end
